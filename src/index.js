...
function createRecepy(data) {
  ...
  let prompt = `User instructions = ${query}. Give me a recepy for this dish, with the ingredients (with quantities) and the steps for preparation until conclusion. Present the ingredients (with quantities) inside a <li></li> element. Te recepy must be divided with divs <div></div> according to each phase of preparation. All the titles of the phases should be written inside a <h3></h3> element.`;
  let context = 'You are a professional brazillian cheff. Your job is to provide recepies according to the user instructions. DO NOT write a tittle to the recepy. You MUST consider the number of people eating before giving the recepy. Be polite. Make the prepare instructions short and objective';
